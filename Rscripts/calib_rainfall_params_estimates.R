...
## Admin1 Results
depts1=list.files(path="Single_node_calibration_output_decay/")
depts2=list.files(path="Dept_admin2_nodes_calibration_output/")
depts2=depts2[-grep("old",depts2)]
depts2=depts2[-grep("RData",depts2)]
depts2.all=list.files(path="chikv_admin2_spatial_output/")
depts2.m=depts2.all[which(!depts2.all %in% depts2)]

depts1.l=list()
for(i in 1:length(depts1)){
  fpath=paste("CHIKV_Calib_Habitat_Decay_Imports_SingleNode_",depts1[i],"/CalibManager.json",sep="")
  calib.j=fromJSON(txt=fpath)
  #outpath=paste("calibs/",depts1[i],"_single.json",sep="")
  #write(calib.j,outpath)
  depts1.l[[i]]=calib.j
}

depts2.l=list()
for(i in 1:length(depts2)){
  fpath=paste("CHIKV_Calib_Habitat_Imports_admin2nodes_Migration_",depts2[i],"/CalibManager.json",sep="")
  calib.j=fromJSON(txt=fpath)
  #outpath=paste("calibs/",depts1[i],"_single.json",sep="")
  #write(calib.j,outpath)
  depts2.l[[i]]=calib.j
...
cnt=1
for(i in 30:32){
  fpath=paste("CHIKV_Calib_Habitat_Multiple_Imports_admin2nodes_Migration_",depts2.m[cnt],"/CalibManager.json",sep="")
  calib.j=fromJSON(txt=fpath)
  #outpath=paste("calibs/",depts1[i],"_single.json",sep="")
  #write(calib.j,outpath)
  depts2.l[[i]]=calib.j
...
"CHIKV_Calib_Habitat_Decay_Imports_SingleNode_"
...
##marginal likelihood for rainfall - use top 100 iterations could also do top 100 weighted by LL
rain.p=calib.j$results$TEMPORARY_RAINFALL

ll.s=(calib.j$results$total+abs(min(calib.j$results$total)))
ll.m=ll.s/sum(ll.s)
rain.m=rain.p*ll.m
