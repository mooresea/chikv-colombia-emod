...
class CHIKVTimeSeriesAnalyzer(BaseComparisonAnalyzer):
    ...
    #filenames = ['output/SpatialReport_New_Reported_Infections.bin','output/SpatialReport_Population.bin','config.json']
    ...
    def apply(self, parser):
        ...
        #infection_data = parser.raw_data[self.filenames[0]]['data']
        ...
        #population = parser.raw_data[self.filenames[1]]['data']
       ...
       # print incidence
        ...
        # infection_weekly = infection_df.groupby(infection_df.index / 7).sum()
        # population_weekly = population_df.groupby(population_df.index / 7).mean()
        ...
        #channel_data = infection_df.merge(population_df, left_index=True,right_index=True)
    ...
    def compare(self, sample):
        ...
        # pop_sample = sample[list(filter(lambda x: 'population' in x, sample.columns))]
        # inf_sample = sample[list(filter(lambda x: 'infections' in x, sample.columns))]
        # #Make sure data sorted by node ID (as string)
        # pop_sample = pop_sample[sorted(pop_sample.columns)]
        # inf_sample = inf_sample[sorted(inf_sample.columns)]
        ...
        # return LL_calculators_dengue.betaBinomial_dengue_modified(rep_data_df.values.tolist(),
        #                                         inf_sample.values.tolist(),pop_sample.values.tolist())
