...
from ColombiaChikvTSSite_SingleNode import ColombiaTSSite
import numpy as np
import scipy as sp
...
climate_file_path = 'D:\\Eradication/InputDataFiles/Colombia_departments_single_nodes/%s/' % site_name
cb.set_input_files_root(climate_file_path)
cb.set_exe_collection('2c2780b0-f96b-e711-9401-f0921c16849d')
#cb.set_experiment_executable('D:\\Eradication\\Executable\\Eradication.exe')
#cb.set_dll_root('D:\\Eradication\\bin')
cb.set_param('Air_Temperature_Filename','Colombia_admin1_popweighted_air_temperature_daily.bin')
cb.set_param('Land_Temperature_Filename','Colombia_admin1_popweighted_air_temperature_daily.bin')
cb.set_param('Rainfall_Filename','Colombia_admin1_popweighted_rainfall_daily.bin')
cb.set_param('Relative_Humidity_Filename','Colombia_admin1_popweighted_humidity_daily.bin')
demo_file_name = "%s_climate_demographics_vector_scaled.json" % site_name
...
for i in range(0,len(demog['Nodes'])):
    ...
    node_pop.append(demog['Nodes'][i]['NodeAttributes']['InitialPopulation'])
...
scaled_pop = np.asarray(node_pop) * scale_factor
...
params.append(create_param('TEMPORARY_RAINFALL', 10, 0.1, 50))
...
for i in range(0, node_cnt):
    for j in range(0, intro_cnt):
        ...
        params.append(create_param('Import_Times_%s_%s' % (i,j), 0.5,0.33,1))
        params.append(create_param('Import_Heights_%s_%s' % (i,j),10,0.001,100))
        params.append(create_param('Import_Widths_%s_%s' % (i,j),5,1,50))
...
def sample_point_fn(cb, params_dict):
    ...
    """
    cb.config["parameters"]["Vector_Species_Params"]["aegypti"]["Larval_Habitat_Types"]["TEMPORARY_RAINFALL"] = params_dict['TEMPORARY_RAINFALL']*pow(10,11)
    ...
    par_heights_import = []
    for i in range(0, node_cnt):
        par_times_import.append([(params_dict['Import_Times_%s_%s' % (i,str(yy))]) for yy in range(0, intro_cnt)])
        par_widths_import.append([(params_dict['Import_Widths_%s_%s' % (i,str(yy))]) for yy in range(0, intro_cnt)])
        par_heights_import.append([(params_dict['Import_Heights_%s_%s' % (i,str(yy))]) for yy in range(0, intro_cnt)])

    # generate time-varying force of infection patterns by serotype
    foi_times = foi_time(par_times_import, par_widths_import, par_heights_import, days_import, node_cnt)
    ...
    for tt in range(0, sim_length - 1):
        for node_id in range(node_cnt):
            if importations[node_id][tt] > 0.0:
                add_OutbreakIndivisualDengue(cb, tt, {}, importations[node_id][tt] / scaled_pop[node_id], 'Strain_1',
                                             [node_ids[node_id]])
...
calib_name = "CHIKV_Calib_Habitat_Decay_Imports_SingleNode_Symptomatic_Low_%s" % site_name
